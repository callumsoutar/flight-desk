...
export function ReturnEquipmentModal({
  ...
  const onSubmit = async () => {
    ...
    if (!parsed.success) {
      toast.error("Please check the return notes")
      return
    }
  ...
  const issuedToName = equipment.issued_to_user
    ? `${equipment.issued_to_user.first_name || ""} ${equipment.issued_to_user.last_name || ""}`.trim() ||
      equipment.issued_to_user.email
    : "Unknown"
  const currentIssuance = equipment.current_issuance
              ...
              <section>
                ...
                <div className="rounded-2xl bg-slate-50/50 border border-slate-100 p-4 space-y-3">
                  <div className="flex justify-between items-center text-sm gap-2">
                    ...
                    <span className="font-bold text-slate-900 text-right">{equipment.name}</span>
                  </div>
                  <div className="flex justify-between items-center text-sm gap-2">
                    <span className="text-slate-500 font-medium">Issued To</span>
                    <span className="font-bold text-slate-900 text-right">{issuedToName}</span>
                  </div>
                  {currentIssuance?.issued_at ? (
                    <div className="flex justify-between items-center text-sm gap-2">
