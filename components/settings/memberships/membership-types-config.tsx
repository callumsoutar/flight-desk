...
export function MembershipTypesConfig() {
  ...
  const [selectedType, setSelectedType] = React.useState<MembershipTypeWithChargeable | null>(null)
  ...
  const [isBenefitsExpanded, setIsBenefitsExpanded] = React.useState(true)
  const [taxRate, setTaxRate] = React.useState(0.15)

  const [editFormData, setEditFormData] = React.useState<MembershipTypeFormData>({
    name: "",
    code: "",
    description: "",
    duration_months: "",
  ...
  const [addFormData, setAddFormData] = React.useState<MembershipTypeFormData>({
    name: "",
    code: "",
    description: "",
    duration_months: "12",
  ...
  const fetchMembershipTypes = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await fetch("/api/membership-types?include_inactive=true")
      if (!response.ok) throw new Error("Failed to fetch membership types")
      const data = (await response.json().catch(() => null)) as { membership_types?: MembershipTypeWithChargeable[] } | null
      setMembershipTypes(Array.isArray(data?.membership_types) ? data.membership_types : [])
    } catch (err) {
  ...
  const fetchMembershipChargeables = async () => {
    try {
      ...
      const mapped: MembershipChargeable[] = rows
        .map((row) => {
      ...
      setMembershipChargeables(mapped)
    } catch {
      // ignore
    }
  }
  ...
  const fetchTaxRate = async () => {
    try {
      ...
      setTaxRate(typeof defaultRate === "number" ? defaultRate : 0.15)
    } catch {
      setTaxRate(0.15)
    }
  }
  ...
  React.useEffect(() => {
    void fetchMembershipTypes()
    void fetchMembershipChargeables()
    void fetchTaxRate()
  }, [])
  ...
  React.useEffect(() => {
    ...
    setEditFormData({
      ...
      duration_months: String(selectedType.duration_months),
      benefits: normalizeBenefits(selectedType.benefits),
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/membership-types", {
        ...
        body: JSON.stringify({
          ...addFormData,
          duration_months: parseInt(addFormData.duration_months, 10),
        }),
      })
  ...
  const handleEdit = async () => {
    ...
    try {
      setSaving(true)
      setError(null)
      const response = await fetch(`/api/membership-types/${encodeURIComponent(selectedType.id)}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...editFormData,
          duration_months: parseInt(editFormData.duration_months, 10),
        }),
      })
  ...
  const handleDelete = async (type: MembershipTypeWithChargeable) => {
    if (
      !confirm(
        `Are you sure you want to delete "${type.name}"? This will deactivate it but preserve historical data.`
      )
    ) {
      return
    }
  ...
  const addBenefitToForm = (formType: "add" | "edit") => {
    ...
    if (formType === "add") {
      setAddFormData((prev) => ({ ...prev, benefits: [...prev.benefits, value] }))
    } else {
      setEditFormData((prev) => ({ ...prev, benefits: [...prev.benefits, value] }))
    }
    setNewBenefit("")
  }
  ...
  const removeBenefitFromForm = (index: number, formType: "add" | "edit") => {
    if (formType === "add") {
      setAddFormData((prev) => ({ ...prev, benefits: prev.benefits.filter((_, i) => i !== index) }))
    } else {
      setEditFormData((prev) => ({ ...prev, benefits: prev.benefits.filter((_, i) => i !== index) }))
    }
  }
  ...
  const calculateTaxInclusiveRate = (rate: number, isTaxable = true) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="h-[650px] flex gap-6">
            ...
            >
              ...
              >
                <div className="flex h-full min-h-0 flex-col bg-white">
                        ...
                        <div className="grid gap-5">
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                setAddFormData((prev) => ({ ...prev, name, code: generateCode(name) }))
                              }}
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) => setAddFormData((prev) => ({ ...prev, code: e.target.value }))}
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) => setAddFormData((prev) => ({ ...prev, duration_months: e.target.value }))}
                              className="h-10 rounded-xl border-slate-200 bg-white px-3 text-base font-medium shadow-none hover:bg-slate-50 focus-visible:ring-0"
                            />
                          </div>
                          ...
                          <div>
                            ...
                            <Select
                              ...
                              onValueChange={(value) =>
                                setAddFormData((prev) => ({
                                  ...prev,
                                  chargeable_id: value === "none" ? null : value,
                                }))
                              }
                            >
                              ...
                              <SelectContent>
                                ...
                                {membershipChargeables.map((chargeable) => (
                                  <SelectItem key={chargeable.id} value={chargeable.id}>
                                    {chargeable.name} (
                                    {formatCurrency(
                                      calculateTaxInclusiveRate(chargeable.rate, chargeable.is_taxable)
                                    )}
                                    )
                                  </SelectItem>
                                ))}
                          ...
                          <div className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50/60 p-3">
                            <Switch
                              ...
                              onCheckedChange={(checked) => setAddFormData((prev) => ({ ...prev, is_active: checked }))}
                            />
                  ...
                  <div className="border-t bg-white px-6 py-4 pb-[calc(1rem+env(safe-area-inset-bottom))] sm:pb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <Button
                        onClick={handleAdd}
                        disabled={
                          saving || !addFormData.name.trim() || !addFormData.code.trim() || !addFormData.duration_months
                        }
                        className="h-10 flex-[1.5] rounded-xl bg-slate-900 text-xs font-bold text-white shadow-lg shadow-slate-900/10 hover:bg-slate-800"
                      >
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-sm font-semibold text-indigo-600">{formatCurrency(price)}</span>
                      </div>
        ...
        <div className="w-1/2 border border-slate-200 rounded-xl p-6 flex flex-col bg-white">
          {selectedType ? (
            <div className="h-full flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto space-y-6 pr-2">
                <section className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5">
                      <Label htmlFor="edit-code" className="text-xs font-medium text-slate-700">
                        Code
                      </Label>
                      <Input
                        id="edit-code"
                ...
                <section className="space-y-4 pt-4 border-t">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <Label htmlFor="edit-duration" className="text-xs font-medium text-slate-700">
                        Duration (Months)
                      </Label>
                      <Input
                        id="edit-duration"
                        ...
                        onChange={(e) => setEditFormData((prev) => ({ ...prev, duration_months: e.target.value }))}
                        className="h-9 text-sm rounded-lg"
                      />
                    </div>
                  </div>
                  <div className="space-y-1.5">
                    <Label htmlFor="edit-chargeable" className="text-xs font-medium text-slate-700">
                      Linked Chargeable
                    </Label>
                    <Select
                      value={editFormData.chargeable_id || "none"}
                      onValueChange={(value) =>
                        setEditFormData((prev) => ({ ...prev, chargeable_id: value === "none" ? null : value }))
                      }
                    >
                      ...
                      <SelectContent>
                        ...
                        {membershipChargeables.map((chargeable) => (
                          <SelectItem key={chargeable.id} value={chargeable.id}>
                            {chargeable.name} (
                            {formatCurrency(calculateTaxInclusiveRate(chargeable.rate, chargeable.is_taxable))})
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
              ...
              <div className="mt-auto pt-4 border-t">
                <Button
                  onClick={handleEdit}
                  disabled={
                    saving || !editFormData.name.trim() || !editFormData.code.trim() || !editFormData.duration_months
                  }
