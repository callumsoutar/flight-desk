...
export function SyllabusConfig() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to void this syllabus? This will make it unavailable for new entries.")) {
      return
    }
  ...
  const filteredSyllabi = useMemo(() => {
    ...
    if (!term) return syllabi
    return syllabi.filter((syllabus) => {
      return (
        syllabus.name.toLowerCase().includes(term) || (syllabus.description || "").toLowerCase().includes(term)
      )
    })
  }, [searchTerm, syllabi])
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      ...
      <p className="text-sm text-slate-600">Manage syllabus structures and training program requirements.</p>
                      ...
                      <div className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50/60 p-3">
                        <Switch
                          ...
                          onCheckedChange={(checked) => {
                            ...
                            } else if (!formData.number_of_exams) {
                          ...
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            {hasExams ? (
                              <div className="flex items-center gap-2">
                                <label className="text-[10px] font-medium text-slate-500">Number:</label>
                                <Input
                                  id="number_of_exams"
                                  type="number"
                                  min="1"
                  ...
                  <TableCell className="font-medium text-slate-900">
                    <div>
                      ...
                      {syllabus.description ? (
                        <div className="text-xs text-slate-500 mt-0.5">{syllabus.description}</div>
                      ) : null}
                    </div>
                  </TableCell>
