...
export function ExperienceTypesConfig() {
  ...
  const openEditDialog = (experienceType: ExperienceType) => {
    ...
    setFormData({
      ...
      is_active: experienceType.is_active ?? true,
    })
  ...
  const filteredTypes = useMemo(() => {
    ...
    if (!term) return experienceTypes
    return experienceTypes.filter((type) => {
      return (
        type.name.toLowerCase().includes(term) || (type.description || "").toLowerCase().includes(term)
      )
    })
                        ...
                        <TableCell className="font-medium text-slate-900">
                          <div>
                            ...
                            {experienceType.description ? (
                              <div className="text-xs text-slate-500 mt-0.5">{experienceType.description}</div>
                            ) : null}
                          </div>
                        </TableCell>
                        <TableCell>
