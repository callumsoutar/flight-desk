...
const EditMaintenanceHistoryModal: React.FC<EditMaintenanceHistoryModalProps> = ({
  ...
  useEffect(() => {
    ...
    fetch(`/api/maintenance-visits?maintenance_visit_id=${maintenanceVisitId}`)
      .then(async (res) => {
        ...
        setHoursAtVisit(
          visit.hours_at_visit !== null && visit.hours_at_visit !== undefined
            ? String(visit.hours_at_visit)
            : ""
        )
        setNotes(visit.notes || "")

        const outDateParsed = visit.date_out_of_maintenance
          ? new Date(visit.date_out_of_maintenance)
          : null
        setDateOutOfMaintenance(outDateParsed && isValidDate(outDateParsed) ? outDateParsed : null)

        setComponentDueHours(
          visit.component_due_hours !== null && visit.component_due_hours !== undefined
            ? String(visit.component_due_hours)
            : ""
        )
        setComponentDueDate(parseDateOnlyFieldToLocalDate(visit.component_due_date))

        setNextDueHours(
          visit.next_due_hours !== null && visit.next_due_hours !== undefined
            ? String(visit.next_due_hours)
            : ""
        )
        ...
        if (visit.component_id) {
          ...
          setComponentName(visit.component?.name || null)
        } else {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      date_out_of_maintenance:
        dateOutOfMaintenance && isValidDate(dateOutOfMaintenance)
          ? dateOutOfMaintenance.toISOString()
          : null,
      component_due_hours: componentDueHours ? parseFloat(componentDueHours) : null,
      component_due_date: toYyyyMmDd(componentDueDate),
      next_due_hours: nextDueHours ? parseFloat(nextDueHours) : null,
      next_due_date: toYyyyMmDd(nextDueDate),
    }
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="relative">
                          <CalendarIcon className="pointer-events-none absolute left-3 top-3 h-3.5 w-3.5 text-slate-400 shrink-0" />
                          <DatePicker
                            date={toYyyyMmDd(componentDueDate)}
                            onChange={(value) => setComponentDueDate(value ? new Date(value) : null)}
                            className={cn(
                              "h-10 w-full rounded-xl border-slate-200 bg-white pl-9 pr-3 text-base font-medium shadow-none hover:bg-slate-50 focus:ring-0",
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="relative">
                            <CalendarIcon className="pointer-events-none absolute left-3 top-3 h-3.5 w-3.5 text-slate-400 shrink-0" />
                            <DatePicker
                              date={toYyyyMmDd(nextDueDate)}
                              onChange={(value) => setNextDueDate(value ? new Date(value) : null)}
                              className={cn(
                                "h-10 w-full rounded-xl border-slate-200 bg-white pl-9 pr-3 text-base font-medium shadow-none hover:bg-slate-50 focus:ring-0",
                ...
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <CalendarIcon className="pointer-events-none absolute left-3 top-3 h-3.5 w-3.5 text-slate-400 shrink-0" />
                      <DatePicker
                        date={toYyyyMmDd(dateOutOfMaintenance)}
                        onChange={(value) => setDateOutOfMaintenance(value ? new Date(value) : null)}
                        className={cn(
                          "h-10 w-full rounded-xl border-slate-200 bg-white pl-9 pr-3 text-base font-medium shadow-none hover:bg-slate-50 focus:ring-0",
