create index if not exists aircraft_ttis_audit_tenant_aircraft_created_idx
on public.aircraft_ttis_audit (tenant_id, aircraft_id, created_at desc, id desc);
...
as $$
  ...
  audit_rows as (
    select
      timezone(p_time_zone, audit.created_at)::date as tech_log_date,
      audit.created_at,
      audit.id,
      case
