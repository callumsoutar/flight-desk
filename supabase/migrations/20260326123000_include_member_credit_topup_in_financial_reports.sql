drop function if exists public.get_financial_transaction_list_report(
  timestamptz,
  timestamptz,
  integer,
  integer
...
create function public.get_financial_transaction_list_report(
  p_start timestamptz,
  p_end timestamptz,
...
begin
  ...
  with filtered_transactions as (
    ...
    where t.tenant_id = v_tenant_id
      ...
      and coalesce(t.metadata->>'transaction_type', '') in (
        'invoice_debit',
        'invoice_payment',
        'member_credit_topup'
      )
  ...
  select
    ...
    coalesce(ft.metadata->>'transaction_type', 'unknown') as transaction_subtype,
    coalesce(ft.invoice_id, ip.invoice_id) as related_invoice_id,
    ft.amount,
    case
      when ft.metadata->>'transaction_type' in ('invoice_payment', 'member_credit_topup')
        then coalesce(
    ...
    coalesce(
      ...
      case
        when ft.metadata->>'transaction_type' = 'invoice_debit' then 'Invoice'
        when ft.metadata->>'transaction_type' = 'member_credit_topup' then 'Member credit top-up received'
        else 'Payment'
      end
    ) as description
...
begin
  ...
  payment_rows as (
    ...
    left join public.invoice_payments ip
      on ip.transaction_id = ft.id
     and ip.tenant_id = v_tenant_id
    where ft.metadata->>'transaction_type' in ('invoice_payment', 'member_credit_topup')
  ),
  sales as (
