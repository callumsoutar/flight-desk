...
-- 1) fix users/tenant_users RLS for member creation and orphan-profile visibility
-- 2) harden record_invoice_payment_atomic tenant binding and role checks

-- ---------------------------------------------------------------------------
-- users + tenant_users policy hardening
-- ---------------------------------------------------------------------------
...
USING (
  (id = (SELECT auth.uid()))
  OR public.users_share_tenant(id)
  OR (
    NOT EXISTS (
      SELECT 1
    ...
    AND EXISTS (
      SELECT 1
      FROM public.tenant_users tu_actor
      JOIN public.roles r ON r.id = tu_actor.role_id
      WHERE tu_actor.user_id = (SELECT auth.uid())
...
-- ---------------------------------------------------------------------------
-- Finance hardening: tenant-bound payment recording
-- ---------------------------------------------------------------------------
