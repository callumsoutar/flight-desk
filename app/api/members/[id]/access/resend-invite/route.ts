...
export async function POST(
  ...
  try {
    ...
    const resendResult = await sendMemberAccessInvitation({
      email: memberUserResult.data.email,
      memberId,
      tenantId,
      purpose: "resend Supabase auth invitation for existing tenant member",
    })

    if (!resendResult.sent) {
      return noStoreJson({ error: resendResult.error }, { status: 400 })
    }
